...
export default function MenuPageHeader() {
  ...
  return (
    <div className="flex sticky top-0 z-30 bg-white px-4 py-3 border-b border-gray-300 shadow mb-1 justify-between items-center">
      ...
      >
        <div
          className={`absolute -top-1.5 -right-1.5 text-xs bg-white rounded-full border-2 border-red-500 duration-300 h-5 w-5 grid place-content-center font-medium text-red-600 ${items.length > 0 ? "opacity-100" : "opacity-0"}`}
        >
          <p>{items.length}</p>
        </div>
